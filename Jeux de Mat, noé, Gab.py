#bienvenue dans le code de terre hantée
...
while en_menu:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            x, y = event.pos
            if 500 <= x <= 700 and 550 <= y <= 575:  #bouton
...
## POUR LES autres ennemis qui apparaisse pas
# Gabin
    class Ennemi2:
            def __init__(self, x, y, taille, points_de_vie, degats):
                self.x = x
                ...
                self.vitesse = 0.5
                self.taille = taille
                self.image1 = pygame.image.load("haut1_chauve_souris.png")
                self.image2 = pygame.image.load("haut2_chauve_souris.png")
                self.image3 = pygame.image.load("haut3_chauve_souris.png")
                self.image4 = pygame.image.load("haut4_chauve_souris.png")
                self.animation_timer = 0
                self.image_index = 0
                self.images = [self.image1, self.image2]
                self.image = pygame.transform.scale(self.images[self.image_index], (taille, taille))
        ...
        def attaquer(self, attaques):
            ...
            if keys[pygame.K_SPACE] and self.attaque_timer <= 0:      # Touche pour la premiere attaque
                ...
                if attaque_direction in self.attaque_images_2:
                    ...
                    attaque = Attaque_2(self.x-25, self.y, attaque_direction, self.degats, attaque_images_2)
...
# NOé ####
    ...
    ennemis = []
    max_enemy_count = 12        ## nbr d'ennemis max
    enemy_spawn_timer = 0
# Noé / Gabin ####################################
    def creer_ennemi():
        x = random.randint(0, 1880)
        y = random.randint(0, 1000)   #1880, 1000
        taille = 40
    ...
    while en_cours:
        ...
        for joueur in joueurs:
            if joueur.jouable:
                ...
                x = max(0, min(x, 1880 - 80))
                y = max(0, min(y, 1000 - 80))
        ...
        if enemy_spawn_timer <= 0 and len(ennemis) < max_enemy_count:
            creer_ennemi()
            enemy_spawn_timer = 56 +4# temps entre le spawn des ennemis (pour les 1ere)
    ...
    pygame.quit()  # quiter en appuyent sur la croix pendant le jeu
pygame.quit()  # quiter en appuyent sur la croix dans le menu
